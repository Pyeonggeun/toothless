				...
				<div class="col">
					...
					<div class="row">
						...
						<div class="col border bg-primary py-4 mx-2 rounded">
							<div class="row">
								<div class="col ms-2">
									<a href="./mj_managePaymentYesPage" class="text-white" style="text-decoration: none;">
									납부 완료
									</a>
								</div>
							</div>
					...
					<div class="row mt-4 py-3 justify-content-between">
						<div class="col-6">
							검색결과 (총 <c:set var="yesCount" value="0" />
										<c:forEach items="${dormSelectedList}" var="item">
										    <c:if test="${item.selectedDto.payment_status eq 'Y'}">
										        <c:set var="yesCount" value="${yesCount + 1}" />
										    </c:if>
					...
					<table class="table table-bordered p-0 text-center align-middle">
					  ...
					  <c:forEach items="${paymentYesList }" var="item">
					  <tbody>
					    <tr>
						  ...
						  <td>${item.paymentYesDto.priority_select}</td>
						  <td>
						  	<c:choose>
						  		<c:when test="${item.paymentYesDto.payment_status == 'Y'}">
						  			<a href="./mj_updatePaymentStatusProcess?page=2&payment_status=N&dorm_application_pk=${item.paymentYesDto.dorm_application_pk}" class="text-black d-grid mx-2" style="text-decoration: none;">
							      		<button type="button" class="fw-bold rounded-0 btn btn-danger btn-sm mb-1">납부취소</button>
							      	</a>
						  		</c:when>
						  		<c:otherwise>
